...
while True:
    ...
    if opcion == "1":
        ...
        print("\nCarrito:")
        # Convierte la lista en un conjunto de forma momentánea para no permitir repetición
        for producto in set(carrito):
            # carrito sigue siendo lista
        ...
        try:
            ...
            if cambio >= 0:
                mensaje_cambio = f"Su cambio es: ${round(cambio, 2)}" if cambio > 0 else "¡Exacto! No se require cambio."
                print(f"Gracias por su compra. {mensaje_cambio}")
                break
            else:
                print("El monto ingresado es insuficiente. Porfavor, ingrese un monto válido.")
